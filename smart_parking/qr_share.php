<?php
/**
 * Public QR share page — NO admin/user login required.
 * This is the link sent to drivers over WhatsApp from the Vehicle Entry
 * page so they can view/download their parking QR on their own phone.
 *
 * Safe to expose without login: the token is a random 40-hex-char secret
 * (see generate_qr_token()) that only someone holding the actual link
 * would know — same trust model as a printed ticket QR. Only the token,
 * plate, slot and entry/booking time are shown; nothing sensitive.
 */
...
// Most recent parking record for this QR (falls back to reservation info).
$stmt = $pdo->prepare(
...
if (!$record) {
    $stmt = $pdo->prepare(
        "SELECT r.booking_date, r.arrival_time, r.status, s.slot_code, s.zone, v.plate_number, v.vehicle_type
         ...
         WHERE r.qr_id = ? ORDER BY r.created_at DESC LIMIT 1"
    );
    $stmt->execute([(int) $qr['qr_id']]);
    $record = $stmt->fetch();
}
...
<title>Your Parking QR — <?= e(APP_NAME) ?></title>
...
.qv-note{font-size:.76rem;color:#7691c0;margin-bottom:1.3rem;line-height:1.5;}
...
<body>
<div class="qv-card">
  <div class="qv-title"><i class="bi bi-qr-code-scan"></i> Your Parking QR</div>
  <div class="qv-sub">Show this at the exit. Same QR every time this vehicle parks here.</div>
  ...
  <div class="qv-note"><i class="bi bi-info-circle"></i> Keep this link — this same QR is reused for this plate on every future visit, so you can save it for next time too.</div>
...
function downloadQr() {
  ...
  a.download = 'parking-qr-' + TOKEN.slice(0, 10) + '.png';
  if (canvas) {
